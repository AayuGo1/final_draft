...
``services.kpi_service`` and the full engineering-analytics chart suite
comes from ``services.chart_service``. No KPI calculation or
...
def render_analytics_section(dataframe: pd.DataFrame, date_column: str | None) -> None:
    """Render the full engineering-analytics chart suite for Ammonia Refrigeration.
    ...
    ui.render_section("Ammonia Refrigeration — Engineering Analytics")
    bundle = chart_service.build_department_analytics_bundle(
        dataframe, title_prefix="Ammonia Refrigeration", date_column=date_column,
    )
    ...
    if bundle["gauges"]:
        ui.render_section("Ammonia Refrigeration — Live Gauges")
    ...
    if bundle["sparklines"]:
        ui.render_section("Ammonia Refrigeration — Meter Sparklines")
    ...
    ui.render_section("Ammonia Refrigeration — Statistics Panel")
    with st.container(border=True):
        ui.render_dataframe(bundle["statistics_table"])

    ui.render_section("Ammonia Refrigeration — Daily Change")
    with st.container(border=True):
...
def render() -> None:
    ...
    date_column = resolve_date_column(ammonia_dataframe, section)
    ...
    render_trend_section(ammonia_dataframe, section, date_column)
    ui.render_divider()

    render_analytics_section(ammonia_dataframe, date_column)
    ui.render_divider()
